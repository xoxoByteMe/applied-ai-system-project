...
---
## Added Features
The Agentic DJ (Agentic Workflow + Logging)
Transforming the app from a static rule-based sorter into an interactive AI agent that dynamically curates playlists.

The Upgrade: Replace the hardcoded classify_song logic with an LLM agent capable of tool-calling.

How it Works: The user inputs a complex prompt (e.g., "I need a 45-minute playlist to help me focus on coding, mostly ambient but keep the energy up at the end"). The agent uses tools to access the song library, calculate the total duration and average energy, and return a structured playlist.
